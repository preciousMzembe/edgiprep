...
class MockController extends GetxController {
  ...
  Future<bool> getData(String testId) async {
    ...
    Map data = await mockService.fetchData(testId);

    if (data['error']) {
      error = true;
    } else {
      Map mockData = data['mockData'];

      testDoneID.value = mockData['id'];

      if (mockData['testInstances'] == null ||
          mockData['testInstances'].isEmpty) {
        return true;
      }
  ...
  Future<void> saveTestScore(double score) async {
    if (answerIds.isNotEmpty) {
      mockService.saveTestScore(testDoneID.value, score, answerIds);
    }
  }
}
