...
class _LearnState extends State<Learn> {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, constants) {
      ...
      return Obx(() {
        final numSubLists = (userController.currentSubjects.length / 2).ceil();
        newSubjectList = List.generate(numSubLists, (index) {
          return userController.currentSubjects.sublist(
              index * 2,
              userController.currentSubjects.length < (index * 2) + 2
                  ? userController.currentSubjects.length
                  : (index * 2) + 2);
        });
        return Column(
